...
type
  ...
  public
    ...
    procedure rec(path:string);
    procedure stopREC();
var
  Video_frm_Main: TVideo_frm_Main;
   CapFName: WideString;
implementation
...
procedure TVideo_frm_Main.A_OptionExecute(Sender: TObject);
begin
  frm_Option.ShowModal;
end;

procedure TVideo_frm_Main.Timer1Timer(Sender: TObject);
var
  Position: int64;
  Rate: Double;
...
procedure TVideo_frm_Main.VMRTextOut(AText: string);
var
...
procedure TVideo_frm_Main.A_PlayExecute(Sender: TObject);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      if FSize.X <> 0 then
        Video_frm_Main.Width := FSize.X + 8;
      if FSize.Y <> 0 then
        Video_frm_Main.Height := FSize.Y + 94;
    end;
    // Change mode
...
procedure TVideo_frm_Main.A_StopExecute(Sender: TObject);
begin
...
procedure TVideo_frm_Main.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  A_Stop.Execute;
end;

procedure TVideo_frm_Main.A_CaptureExecute(Sender: TObject);
begin
  A_Stop.Execute;
  if CapFName = '' then
  begin
...
procedure rec(path:string);
begin
  Video_frm_Main.A_Stop.Execute;
  Video_frm_Main.A_Capture.Checked := True;
  CapFName := path;
  Video_frm_Main.A_Play.Execute;
end;
procedure stopREC();
begin
  Video_frm_Main.A_Stop.Execute;
  Video_frm_Main.A_Capture.Checked := False;
  CapFName := '';
  Video_frm_Main.A_Play.Execute;
end;

procedure TVideo_frm_Main.A_OnTopExecute(Sender: TObject);
begin
...
procedure TVideo_frm_Main.A_FullScreenExecute(Sender: TObject);
begin
  A_FullScreen.Checked := not A_FullScreen.Checked;
  VideoWindow1.FullScreen := A_FullScreen.Checked;
end;

procedure TVideo_frm_Main.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TVideo_frm_Main.A_ExitExecute(Sender: TObject);
begin
  Close;
end;

procedure TVideo_frm_Main.A_BitmapExecute(Sender: TObject);
var
  ABitmap: TBitmap;
...
procedure TVideo_frm_Main.A_FileExecute(Sender: TObject);
begin
